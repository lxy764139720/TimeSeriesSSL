...
def main():
    ...
    trainset = jn_dataset(r=args.r, root_dir='./JN_dataset', mode='train', noise_mode=args.noise_mode,
                            transform=transform1, noise_file='%s/%.1f_%s.json' %
                                                             (args.data_path, args.r, args.noise_mode))
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size,
                                              shuffle=True, num_workers=args.workers, pin_memory=True)
    testset = jn_dataset(r=args.r, root_dir='./JN_dataset', mode='test', noise_mode=args.noise_mode,
...
def adjust_learning_rate(optimizer, epoch):
    ...
    # stage2=295, num_epochs=300
    if epoch < args.stage2:
        lr = args.lr
    else:
        lr = args.lr
    # elif epoch < (args.num_epochs - args.stage2) // 3 + args.stage2:  # 200
    #     lr = args.lr2
    # elif epoch < 2 * (args.num_epochs - args.stage2) // 3 + args.stage2:  # 250
    #     lr = args.lr2 // 2
    # else:
    #     lr = args.lr2 // 2
